    ...
    for (c = 0; c < 4; c++) {
        ...
        for (r = 0; r < 4; r++) {
               ...
               state[r][c] = FFmul(0x0e, t[r])
                              ...
                              ^ FFmul(0x09, t[(r+3)%4]);
#else
            state[r][c] = FFmul_fast(0x0e, t[r]) ^ FFmul_fast(0x0b, t[(r + 1) % 4]) ^ FFmul_fast(0x0d, t[(r + 2) % 4]) ^ FFmul_fast(0x09, t[(r + 3) % 4]);
...
int AESModeOfOperation::Encrypt(const ZUINT1 *_in, int _length, ZUINT1 *_out) {
    ...
    for (int j = 0; j < rounds; ++j) {
        ...
        if (end * 1.0 / _length > lastLen) {
            lastLen = end * 1.0 / _length + 0.01;
            printf("\rdeal %2.2f%%...", end * 1.0 / _length * 100);
            fflush(stdout);
        }
    }
